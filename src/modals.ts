...
export class TaskModal extends Modal {
  ...
  validateInputs(taskData: TaskData): boolean {
    ...
    if ((taskData.startTime && !taskData.endTime) || (!taskData.startTime && taskData.endTime)) {
      new Notice("Both Start Time and End Time must be provided if one is set.");
      return false;
    }

    if (taskData.startTime && taskData.endTime && taskData.startTime.isAfter(taskData.endTime)) {
